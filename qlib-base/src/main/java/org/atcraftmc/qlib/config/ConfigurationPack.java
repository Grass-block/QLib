...
import org.atcraftmc.qlib.util.FileUtil;
...
public abstract class ConfigurationPack {
    ...
    public File getFile() {
        return FileUtil.tryReleaseAndGetFile(this.provider.platform(), getTemplateDirectory(), getStorageDirectory());
    }

    private void merge(ConfigurationSection current, ConfigurationSection mod) {
        if (mod != null) {
            for (var key : mod.getKeys(false)) {
                if (mod.isConfigurationSection(key)) {
                    continue;
                }
                if (mod.get(key) == null) {
                    continue;
